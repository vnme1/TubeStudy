...
function getVideoData() {
    ...
    // 영상 제목, 채널명 추출
    const titleElement = document.querySelector('h1.style-scope.ytd-watch-metadata, h1.style-scope.ytd-video-primary-info-renderer');
    ...
    return {
        ...
        channel: channelElement ? channelElement.innerText.trim() : 'Unknown Channel',
...
async function sendDataToServer() {
    ...
    if (data.videoId && data.duration > 0) {
        ...
        console.log(`[TubeStudy] 전송 중: ${data.title} (${Math.floor(data.currentTime)}/${Math.floor(data.duration)}s)`);

        try {
            // 포트 8085로 전송 (수정된 포트)
            const response = await fetch('http://localhost:8085/api/tracker/sync', {
                method: 'POST',
            ...
            if (response.ok) {
                ...
                if (syncResponse.requiresNotification) {
                    console.warn(`[TubeStudy] 딴짓 감지! 메시지: ${syncResponse.message}`);
